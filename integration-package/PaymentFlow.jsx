  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentMethod === 'mercadopago') {
        ...
        // Actualizar stock automáticamente para pedidos en efectivo
        try {
          const functions = getFunctions();
          const updateStock = httpsCallable(functions, 'updateStock');
          await updateStock({ orderId });
          console.log('Stock updated successfully for cash order:', orderId);
        } catch (stockError) {
          console.error('Error updating stock for cash order:', stockError);
          // No interrumpir el flujo, pero loguear el error
        }
